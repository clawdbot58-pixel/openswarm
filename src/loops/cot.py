...
class CoTLoop(BaseLoop):
    """Chain-of-thought reasoning - single call with reasoning prefix."""

    def __init__(self, model_client: LLMClient):
        """Initialize the CoT loop.
    ...
    def cost_multiplier(self) -> float:
        """Cost multiplier is 1.0 for CoT loop."""
        return 1.0
    ...
    async def run(
        ...
        """Execute the chain-of-thought loop.
        ...
        cot_task = "Let's think step by step. " + task
        ...
        return LoopResult(
            ...
            intermediate_outputs=[
                {"type": "cot_reasoning", "content": response.content}
            ],
        )

    def _build_system_prompt(self, preamble: dict[str, Any]) -> str:
        """Build system prompt from preamble with CoT instructions.
        ...
        prompt += f"\n# REASONING\n"
        prompt += "Think step by step. Show your reasoning process.\n"
